...
public:
	...
	// 내부 기능 functions
	Book* findBook(int id); // 포인터 반환 -> 없으면 nullptr
	void save(string filename); // 파일 저장용

	// 외부 기능 functions
	void insertBook(string name, string password); // 도서 등록 : 여기서 권한 검사, bookCount 통해 Book() 생성 -> addBook 으로 DB에 추가
